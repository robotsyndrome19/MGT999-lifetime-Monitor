...
import { LangContext, useLang, translations, type Lang } from './i18n';
      ...
      <div className={`min-h-screen flex flex-col transition-colors duration-500 font-sans selection:bg-red-500/30 ${
        ...
        <nav className={`sticky top-0 z-50 border-b backdrop-blur-xl transition-all duration-500 ${
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1 sm:gap-2 relative">
              <button
                onClick={toggleLang}
                className={`px-2 py-1 rounded-lg text-xs font-bold tracking-widest transition-colors ${
                  theme === 'dark' ? 'hover:bg-zinc-900 text-zinc-400' : 'hover:bg-zinc-100 text-zinc-500'
                }`}
                    ...
                    <>
                      ...
                      >
                        <div className={`p-4 border-b flex items-center justify-between ${theme === 'dark' ? 'border-zinc-800' : 'border-zinc-100'}`}>
                          <span className={`text-xs font-bold uppercase tracking-widest ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{t.notifications.title}</span>
                          <span className="text-[10px] text-red-500 font-bold uppercase tracking-widest">{unreadNotificationsCount} {t.notifications.new}</span>
                        </div>
                ...
                <div className="p-6 flex flex-col h-full">
                  <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-3">
                      ...
                      <span className={`font-bold uppercase tracking-tight ${theme === 'dark' ? 'text-white' : 'text-zinc-900'}`}>{t.nav.menu}</span>
                    </div>
                    <button
                      onClick={() => setIsMobileMenuOpen(false)}
                  ...
                  <div className="mt-4">
                    <button
                      onClick={toggleLang}
                      className={`w-full flex items-center justify-center p-3 rounded-xl text-xs font-bold uppercase tracking-widest transition-colors border ${
                        theme === 'dark'
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-10 min-w-0">
            ...
            >
              ...
              <div className={`text-2xl sm:text-3xl font-light font-mono transition-colors duration-500 ${
                ...
                {data.robot_lifetime.runtime.toFixed(2)} <span className="text-sm text-zinc-500">{t.stats.hrs}</span>
              </div>
            </motion.div>
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <span className={`text-xs font-semibold uppercase tracking-widest ${
                  theme === 'dark' ? 'text-zinc-500' : 'text-zinc-400'
                }`}>{t.stats.activeAxes}</span>
                <Cpu className={theme === 'dark' ? 'text-red-500' : 'text-red-700'} />
              </div>
              <div className={`text-2xl sm:text-3xl font-light font-mono transition-colors duration-500 ${
                theme === 'dark' ? 'text-white' : 'text-zinc-900'
              }`}>
                {String(Object.keys(data.robot_axis).length).padStart(2, '0')} <span className="text-sm text-zinc-500">{t.stats.online}</span>
              </div>
            </motion.div>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="md:col-span-1 md:order-2 lg:order-none lg:col-span-3 flex flex-col gap-6">
                  <div className={`rounded-2xl border overflow-hidden transition-all duration-500 flex flex-col h-full ${
                    ...
                    <div className={`p-4 flex items-center justify-between font-bold uppercase tracking-widest text-white transition-colors duration-500 ${
                    ...
                    <div className="p-6 flex-grow flex flex-col items-center justify-center gap-6">
                      <div className="w-full aspect-square relative flex items-center justify-center p-4">
                        <img
                          src="/controller.png"
                          alt="KUKA Controller"
                          className={`max-w-full max-h-full object-contain drop-shadow-2xl transition-all duration-500 ${!isRobotOn ? 'opacity-40 grayscale' : ''}`}
                          onError={(e) => {
            ...
            {activeTab === 'analytics' && (
              <motion.div
                ...
                exit={{ opacity: 0, x: 20 }}
              >
                <Analytics data={data} history={analyticsHistory} theme={theme} isRobotOn={isRobotOn} />
              </motion.div>
            )}
        ...
        <div className={`fixed bottom-0 left-0 right-0 z-40 border-t backdrop-blur-xl transition-all duration-500 ${
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 h-12 flex items-center gap-2 overflow-hidden">
            ...
            >
              <ChevronLeft className="w-3.5 h-3.5 flex-shrink-0" />
              <span className="hidden xs:inline">{t.nav.backHome}</span>
              <span className="xs:hidden">{t.nav.home}</span>
            </button>
            ...
            >
              <ExternalLink className="w-3.5 h-3.5 flex-shrink-0" />
              <span className="hidden sm:inline">{t.nav.goPalletizer}</span>
              <span className="sm:hidden">{t.nav.palletizer}</span>
            </a>
          </div>
        </div>
...
function Modal({ type, logs, onClose, theme }: ModalProps) {
  ...
  const modalConfig = {
    privacy: {
      ...
      body: (
        <div className="space-y-4 text-sm leading-relaxed">
          ...
          <h4 className="font-bold uppercase tracking-tight">{t.modal.privacy.h1}</h4>
          <p>{t.modal.privacy.p1}</p>
          <h4 className="font-bold uppercase tracking-tight">{t.modal.privacy.h2}</h4>
          <p>{t.modal.privacy.p2}</p>
          <h4 className="font-bold uppercase tracking-tight">{t.modal.privacy.h3}</h4>
          <p>{t.modal.privacy.p3}</p>
        </div>
      ),
    },
    logs: {
      title: t.modal.systemLogsTitle,
    ...
    support: {
      ...
      body: (
        <div className="space-y-4 text-sm leading-relaxed">
          <p>{t.modal.support.intro}</p>
          <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-zinc-800/50 border-zinc-700' : 'bg-zinc-100 border-zinc-200'}`}>
            <p className="font-bold text-red-500 mb-1">{t.modal.support.emergencyHotline}</p>
            <p className="text-lg font-mono">+1 (800) RED-LINE</p>
          </div>
          <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-zinc-800/50 border-zinc-700' : 'bg-zinc-100 border-zinc-200'}`}>
            <p className="font-bold text-red-500 mb-1">{t.modal.support.technicalDocs}</p>
            <a href="#" className="underline hover:text-red-500 transition-colors">docs.redline-industrial.com</a>
          </div>
          <p className="text-xs text-zinc-500 italic">{t.modal.support.note}</p>
        </div>
      ),
    },
  };

  const active = modalConfig[type];
      ...
      >
        ...
        <div className="p-3 grid grid-cols-3 gap-1 divide-x divide-zinc-500/20">
          <div className="text-center px-1">
            <div className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1">{t.axis.temp}</div>
            <div className={`text-xs font-mono leading-tight ${theme === 'dark' ? 'text-zinc-300' : 'text-zinc-700'}`}>{data.temp.toFixed(1)}°C</div>
          </div>
          <div className="text-center px-1">
            <div className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1">{t.axis.torque}</div>
            <div className={`text-xs font-mono leading-tight ${theme === 'dark' ? 'text-zinc-300' : 'text-zinc-700'}`}>{data.torque.toFixed(1)}%</div>
          </div>
          <div className="text-center px-1">
            <div className="text-[10px] uppercase tracking-wider text-zinc-500 mb-1">{t.axis.current}</div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-zinc-500">
            ...
            <span className="text-[10px] uppercase tracking-widest font-medium">{t.axis.temp}</span>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-zinc-500">
            ...
            <span className="text-[10px] uppercase tracking-widest font-medium">{t.axis.torque}</span>
          </div>
          <span className={`text-sm font-mono transition-colors duration-500 ${
            theme === 'dark' ? 'text-white' : 'text-zinc-900'
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-zinc-500">
            ...
            <span className="text-[10px] uppercase tracking-widest font-medium">{t.axis.current}</span>
          </div>
          <span className={`text-sm font-mono transition-colors duration-500 ${
            theme === 'dark' ? 'text-white' : 'text-zinc-900'
      ...
      <div className="space-y-4">
        <div>
          <div className="flex justify-between text-[10px] uppercase tracking-widest font-medium text-zinc-500 mb-2">
            <span>{t.cylinder.lifetimeUsage}</span>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-[9px] text-zinc-600 uppercase tracking-widest mb-1">{t.cylinder.actual}</div>
            <div className={`text-xs font-mono transition-colors duration-500 ${
              theme === 'dark' ? 'text-white' : 'text-zinc-900'
            }`}>{data.lifetime_act.toLocaleString()}</div>
          </div>
          <div>
            <div className="text-[9px] text-zinc-600 uppercase tracking-widest mb-1">{t.cylinder.limit}</div>
            <div className={`text-xs font-mono transition-colors duration-500 ${
              theme === 'dark' ? 'text-white' : 'text-zinc-900'
